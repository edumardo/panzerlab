...
def render_translation_page(pdf, content, regular, bold, italic):
    ...
    # A page legitimately has no heading of its own (e.g. a drawing plate
    # whose only text is a figure caption, or a mid-table continuation) --
    # fall back to a plain "(untitled page)" label rather than crashing on
    # a None title.
    en_title = content["titles"].get(LANGUAGES[0]) or "(untitled page)"
    es_title = content["titles"].get(LANGUAGES[1]) or "(página sin título)"
    rule_y = draw_header(
        pdf, en_title, content["page"], regular, bold,
...
def main():
    ...
    pdf.setTitle(f"D. 652/50a - {args.section} - facsimile + bilingual translation")
