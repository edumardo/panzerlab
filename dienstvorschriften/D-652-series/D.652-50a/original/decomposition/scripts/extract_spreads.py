...
The source PDF stores the photographed spread and the diagonal watermark as
separate image objects.  The photographed spread is selected by pixel area,
without rendering the PDF page, so the watermark is never composited.
"""
...
def iter_image_xobjects(resources, prefix: str = "") -> Iterator[tuple[str, object]]:
    """Yield raw image XObjects, including images nested in Form XObjects."""
...
def decode_xobject(obj) -> tuple[Image.Image, bytes | None]:
    """Decode an image while deliberately ignoring any /SMask watermark."""
    ...
    if "/DCTDecode" in filters:
        # A lone DCT stream is already a JPEG.  If Flate precedes DCT, get_data()
        # removes Flate while DCTDecode intentionally leaves JPEG bytes intact.
...
def extract_page(page, page_number: int, output_dir: Path, thumb_width: int) -> dict:
            ...
            candidates.append(
                {
                    ...
                    "filters": [str(value) for value in (obj.get("/Filter") if isinstance(obj.get("/Filter"), list) else [obj.get("/Filter")])],
                    "image": im,
                    "jpeg_data": jpeg_data,
                }
            )
        except Exception as exc:  # retain diagnostics for unusual PDF objects
            candidates.append(
                {
                    "index": index,
                    "name": name,
    ...
    usable = [candidate for candidate in candidates if candidate["area"] > 0]
    if not usable:
        raise RuntimeError(f"No decodable raster image found on PDF page {page_number}")

    selected = max(usable, key=lambda candidate: candidate["area"])
...
def main() -> None:
    ...
    for page_number in range(args.start, end + 1):
        record = extract_page(
            reader.pages[page_number - 1], page_number, args.output_dir, args.thumb_width
        )
        records.append(record)
        selected = record["selected"]
        print(
            f"{page_number:03d}: {selected['width']}x{selected['height']} "
            f"from {len(record['candidates'])} image object(s)"
        )
