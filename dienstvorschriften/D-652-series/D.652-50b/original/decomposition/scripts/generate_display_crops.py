...
manifest into the global `manifest.json` (the archive validator requires the two
to match exactly). Never touches `source.jpg` itself, so it is idempotent and
safe to re-run after a re-scan or a crop-heuristic change.

Heuristic: the aged cream paper is warm (R notably greater than B); the stone/
marble scan background is cool/neutral (R close to B). For each edge, scan a
bounded window inward and find the steepest warmth rise (background -> paper).
"""
...
def process_page(page_dir: Path) -> dict:
    """Returns the updated local page manifest, so the caller can mirror it
    into the global manifest.json (the archive validator requires the two to
    match exactly)."""
    ...
    print(f"{page_dir.name}: {image.size} -> {cropped.size} (box={left},{top},{right},{bottom})")
    return manifest
...
def main():
    ...
    for entry in global_manifest["pages"]:
        ...
        if page_number in updated_by_page:
            entry.clear()
