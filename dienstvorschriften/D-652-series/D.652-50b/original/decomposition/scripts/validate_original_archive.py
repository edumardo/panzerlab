...
def main() -> None:
    ...
    if [entry["page"] for entry in pages] != expected_pages:
        errors.append(f"Global page sequence is not exactly 1-{len(pages)}")

    spread_count = len(list((ORIGINAL / "assets" / "spreads").glob("spread_*.jpg")))
    if spread_count != manifest["source_pdf_pages"]:
        errors.append(f"Clean spread count ({spread_count}) does not match source_pdf_pages ({manifest['source_pdf_pages']})")
    ...
    extraction_manifests = sorted((ORIGINAL / "assets").glob("extraction_*.json"))
    for extraction_manifest in extraction_manifests:
    ...
    for entry in pages:
        ...
        if content["id"] != f"{DOCUMENT_SLUG}-page-{page:03d}" or content["page"] != page:
    ...
    if figure_numbers and sorted(figure_numbers) != list(range(1, len(figure_numbers) + 1)):
        errors.append(f"Figure sequence is not exactly Bild 1-{len(figure_numbers)}")
    ...
    print(f"Figures: {len(figure_numbers)} (Bild 1-{len(figure_numbers)})" if figure_numbers else "Figures: 0")
