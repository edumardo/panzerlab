"""Export the entire D.652-41a document as a single bilingual PDF.
...
page), then every section S01-S06 and G in manifest order
(facsimile+translation per page). The document's only canonical bilingual
output is bilingual/D.652-41a_bilingual_full_v<version>.pdf -- no per-section
PDFs are kept.

Each facsimile page takes the orientation of its own source scan (29 of the
101 scans are landscape); translation pages and the title page stay A4
portrait, which is the series-wide standard. See export_facsimile_pdf.py for
why orientation is decided per page rather than per section.

Overwrites --output in place. Requires validated status for transcription,
en-GB and es-ES on every page unless --allow-draft is passed.
"""
...
def _section_dir(root, section_id):
    return root / "frontmatter" if section_id == "FrontMatter" else root / "sections" / section_id
...
def main():
    ...
    title_geom = Geometry(PORTRAIT)
    pdf = canvas_module.Canvas(str(args.output), pagesize=title_geom.pagesize, pageCompression=1)
    pdf.setTitle("D. 652/41a - complete bilingual facsimile + translation")
    pdf.setAuthor("PanzerLab")
    pdf.setSubject("Facsimile export generated from the canonical JSON decomposition")

    render_title_page(pdf, TITLE_PAGE_SPEC, regular, bold, italic,
                      title_geom.width, title_geom.height)

    has_draft = False
    total_pages = 0
    total_translations = 0

    for section_id, page_range in manifest["sections"].items():
        section_dir = _section_dir(root, section_id)
        page_numbers = list(range(page_range[0], page_range[1] + 1))
        entries, draft = _load_entries(section_dir, page_numbers, args.allow_draft)
        has_draft = has_draft or draft
        total_translations += _render_entries(
            pdf, entries, regular, bold, italic,
            skip_blank_translation=(section_id == "FrontMatter"),
        )
